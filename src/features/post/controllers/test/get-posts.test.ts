...
describe('Get', () => {
  ...
  describe('posts', () => {
    it('should send correct json response if posts exist in cache', async () => {
      ...
      expect(PostCache.prototype.getPostsFromCache).toHaveBeenCalledWith('posts:post', 0, 10);
  ...
  describe('postWithImages', () => {
    it('should send correct json response if posts exist in cache', async () => {
      ...
      expect(PostCache.prototype.getPostsWithImagesFromCache).toHaveBeenCalledWith('posts:post', 0, 10);
      expect(res.status).toHaveBeenCalledWith(200);
      expect(res.json).toHaveBeenCalledWith({
