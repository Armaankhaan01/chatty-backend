...
import { authMockRequests, authMockResponse, authUserPayload } from '@root/mocks/auth.mock';
...
describe('CurrentUser', () => {
  ...
  describe('token', () => {
    it('should set session token to null and send correct json response', async () => {
      const req: Request = authMockRequests({}, { username: USERNAME, password: PASSWORD }, authUserPayload) as Request;
    ...
    it('should set session token and send correct json response', async () => {
      const req: Request = authMockRequests({ jwt: '12djdj34' }, { username: USERNAME, password: PASSWORD }, authUserPayload) as Request;
      const res: Response = authMockResponse();
