...
describe('Get', () => {
  ...
  describe('all', () => {
    it('should send success json response if users in cache', async () => {
      const req: Request = authMockRequests({}, {}, authUserPayload, { page: '1' }) as Request;
  ...
  describe('profile', () => {
    it('should send success json response if user in cache', async () => {
      const req: Request = authMockRequests({}, {}, authUserPayload) as Request;
      const res: Response = authMockResponse();
  ...
  describe('profileAndPosts', () => {
    it('should send success json response if user in cache', async () => {
      const req: Request = authMockRequests({}, {}, authUserPayload, {
      ...
      expect(UserCache.prototype.getUserFromCache).toHaveBeenCalledWith(`${req.currentUser?.userId}`);
      expect(PostCache.prototype.getUserPostsFromCache).toHaveBeenCalledWith('posts:post', parseInt(req.params.uId, 10));
      expect(res.status).toHaveBeenCalledWith(200);
      expect(res.json).toHaveBeenCalledWith({
    ...
    it('should send success json response if user in database', async () => {
      ...
      expect(postService.getPosts).toHaveBeenCalledWith({ username: userName }, 0, 1000, { createdAt: -1 });
      expect(res.status).toHaveBeenCalledWith(200);
      expect(res.json).toHaveBeenCalledWith({
        message: 'Get user profile and posts',
        user: existingUser,
